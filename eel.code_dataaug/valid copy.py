...
def compute_anomaly_score(score, mode='energy'):
    ...
    if mode == 'energy':
        ...
        normalized_entropy=entorpy_score/entorpy_score.max()
        energy=torch.sum(torch.exp(score),dim=0)
        normalize_energy=energy/energy.max()
        ...
        anomaly_score = -(1. * torch.logsumexp(score, dim=0))+entorpy_score*1
...
def valid_anomaly(model, engine, test_set, data_name=None, iteration=None, my_wandb=None, logger=None,
                  upload_img_num=9, measure_way="energy"):
    if engine.local_rank <= 0:
        logger.info("validating {} dataset with {} ...".format(data_name, measure_way))
    ...
    with torch.no_grad():
        with autocast(dtype=torch.float16):
            ...
            for idx in tbar:
                ...
                visual[(label != test_set.train_id_out) & (label != test_set.train_id_in)] = 0 #void类=0
