...
import sys
sys.path.append("/home/xias0002/mask2former/Mask2Former-main/RPL")
from extract_object_postprocess import mix_objects
class Preprocess(object):
    def __init__(self, config):
        ...
        self.ood_scale_array = config.ood_train_scale_array #0.25,0.5,0.75,1,1.25倍

    def inlier_transform(self, img_, gt_): #旋转裁切之类的预处理
    ...
    def outlier_transform(self, img_, gt_):
        img_ = normalize(img_, self.img_mean, self.img_std)
        scaled_img_, scaled_gt_, _ = random_scale(img_.copy(), gt_.copy(), self.ood_scale_array) #对图像的大小进行了随机裁剪，然后再填充，可以生成大小不同的target
        ...
        return img_, gt_, scaled_img_, scaled_gt_ #大小的变换

    def __call__(self, city_img, city_gt, ood_img, ood_gt, anomaly_mix_or_not):
    # def __call__(self, city_img, city_gt):
        ...
        if anomaly_mix_or_not:
            city_mix_img, city_mix_gt = self.mix_object(current_labeled_image=city_img.copy(),
                                                        ...
                                                        cut_object_mask=scaled_ood_gt)  #取出bounding box直接copy paste进去
        else:
            city_mix_img = numpy.zeros_like(city_img)
            city_mix_gt = numpy.zeros_like(city_gt)
        # add_cp=random.randint(0,2)
        # if add_cp!=0:
        #     city_mix_img=city_img
        #     city_mix_gt=city_gt
        # city_mix_img,city_mix_gt=mix_objects(city_mix_img,city_mix_gt)
        # city_mix_img = normalize(city_mix_img, self.img_mean, self.img_std)
        # return city_img.transpose(2, 0, 1), city_gt
    ...
    def mix_object(self, current_labeled_image=None, current_labeled_mask=None,
        ...
        cut_object_image = cut_object_image[y1:y2, x1:x2, :] #把有物体的那一块提取出来
        idx = numpy.transpose(numpy.repeat(numpy.expand_dims(cut_object_mask, axis=0), 3, axis=0), (1, 2, 0)) #与图片同形状
        ...
        return current_labeled_image, current_labeled_mask #异常物体的label是254
