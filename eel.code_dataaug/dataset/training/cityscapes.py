...
class Cityscapes(torch.utils.data.Dataset):
    ...
    def __init__(self, root: str = "/path/to/you/root", split: str = "val", mode: str = "gtFine",
        ...
        city_names= os.listdir(img_dir)
        for cities in city_names:
            img_real_dir= os.path.join(img_dir, cities)
            target_real_dir=os.path.join(target_dir, cities)

            for file_name in os.listdir(img_real_dir):
                target_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0],
                                            self._get_target_suffix(self.mode, target_type))
                self.images.append(os.path.join(img_real_dir, file_name))
                self.targets.append(os.path.join(target_real_dir, target_name))
