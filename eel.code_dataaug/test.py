...
from dataset.validation.Diff_anomaly import Diff_anomaly
...
def main(gpu, ngpus_per_node, config, args):
    ...
    segment_me_obstacle = SegmentMeIfYouCan(split='road_obstacle', root=config.segment_me_root_path,
                                            transform=transform)
    diff_amy = Diff_anomaly(split='', root=config.diff_amy_root_path, transform=transform)
    diff_amy_lowres = Diff_anomaly(split='', root=config.diff_amy_lowres_root_path, transform=transform)
    ...
    valid_anomaly(model=model, engine=engine, iteration=0, test_set=diff_amy_lowres,
                  data_name='diff_amy_lowres', my_wandb=vis_tool, logger=logger,
                  measure_way=config.measure_way)

    valid_anomaly(model=model, engine=engine, iteration=0, test_set=diff_amy,
                  data_name='Diff_anomaly', my_wandb=vis_tool, logger=logger,
                  measure_way=config.measure_way)
