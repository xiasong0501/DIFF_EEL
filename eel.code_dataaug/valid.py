...
def flatten_and_sort_2D_array(arr_2d):
    # Step 1: Flatten the 2D array into a 1D array
    arr_2d=np.array(arr_2d.cpu())
    # flattened_array = [element for sublist in arr_2d for element in sublist]
...
def compute_anomaly_score(score, mode='energy'):
    ...
    if mode == 'energy':
        ...
        energy= -(1. * torch.logsumexp(score, dim=0))
       
       
        norm_value=0
        # sorted_energy=flatten_and_sort_2D_array(energy)
        # length=len(sorted_energy)
        # norm_value=np.mean(sorted_energy[int(length*0.2):int(length*0.3)])
        ...
        #normalize_energy=normalize_energy1/normalize_energy1.max()
        ...
        anomaly_score = energy*1+entorpy_score*1-norm_value
        # anomaly_score = -normalize_energy1*1+normalized_entropy*0.2-normalize_energy*0
    ...
    #assert anomaly_score.shape==1, anomaly_score.shape
...
def valid_anomaly(model, engine, test_set, data_name=None, iteration=None, my_wandb=None, logger=None,
                  upload_img_num=30, measure_way="energy"):
    if engine.local_rank <= 0:
        logger.info("validating {} dataset with {} ...".format(data_name, measure_way))
    ...
    with torch.no_grad():
        with autocast(dtype=torch.float16):
            ...
            for idx in tbar:
                ...
                visual[(label != test_set.train_id_out) & (label != test_set.train_id_in)] = -10 #void类=0
    ...
    if engine.local_rank <= 0:
       ...
       # assert 1==0,[anomaly_scores,ood_gts]
        curr_info['{}_auroc'.format(data_name)] = roc_auc
        curr_info['{}_fpr95'.format(data_name)] = fpr
        curr_info['{}_auprc'.format(data_name)] = prc_auc
