...
def declare_settings(config_file, logger, engine):
    ...
    logger.info("city-context image: {}x{}, ood-context image: {}x{}".format(config_file['city_image_height'],
...
def main(gpu, ngpus_per_node, config, args):
    ...
    if engine.local_rank <= 0:
        ...
        if args.gcloud:
            ...
            download_checkpoint("./ckpts/", 'pretrained_ckpts/cityscapes_best.pth')
    ...
    #validator=None
    # define the trainer
    trainer = Trainer(engine=engine, loss1=energy_loss, validator=validator,
                      lr_scheduler=lr_policy, ckpt_dir=config.saved_dir, tensorboard=visual_tool,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpus', default=3, type=int, help="gpus in use")
    ...
    parser.add_argument('--online', action="store_true", help="wandb online or not")
    parser.add_argument('--prob', default=1., type=float, help="sample fetch-prob for OE")
    args = parser.parse_args()

    #print(sys.path)
    # if args.dgx:
    #     config.city_root_path = "./city_scape"
    #     config.coco_root_path = "./coco"
    #     config.fishy_root_path = "./fishyscapes"
    #     config.segment_me_root_path = "./segment_me"
    #     config.road_anomaly_root_path = './road_anomaly'
    #     config.lost_and_found_root_path = './lost_and_found'
    #     config.saved_dir = "./ckpts"
