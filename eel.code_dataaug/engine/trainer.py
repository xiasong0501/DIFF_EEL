...
class Trainer:
    ...
    def train(self, model, epoch, train_sampler, train_loader, optimizer):
        ...
        #scaler= torchcuda.amp.GradScaler()
        # with autocast(dtype=torch.float16):
        for batch_idx in tbar:
            city_imgs, city_targets, city_mix_imgs, city_mix_targets, ood_imgs, ood_targets = next(train_loader)
            CoCo_or_not=random.randint(0, 1)
            if CoCo_or_not > 0:
                city_mix_imgs, city_mix_targets=city_imgs, city_targets
            ...
            city_vanilla_logits, city_mix_logits = model(city_mix_imgs) #模型输出vanilla_logits 和 city_mix_logits(应该是单独的和混合之间的输出)
            # torch.save(city_vanilla_logits,'vanilla.pt')
            # torch.save(city_mix_logits,'mix.pt')
            # energy training
            ...
            #print(loss_dict["entropy_part"],loss_dict["reg"],loss_dict["energy_part"] * self.energy_weight)
            #assert 1==0, [loss_dict["entropy_part"],loss_dict["reg"],loss_dict["energy_part"] * self.energy_weight]
            ...
            if self.engine.local_rank <= 0:
                ...
                if curr_idx % 40 == 0:
                    self.tensorboard.upload_wandb_info(current_step=curr_idx, info_dict=curr_info, measure_way="energy")
            ...
            #if curr_idx % self.validator.eval_iter == 0 and epoch >= 2:
            if curr_idx % 500 == 0 and epoch >= 1 :
                    #del model

                self.freeze_model_parameters(model)
                model=model.cuda()
                # with torch.no_grad(): 
                #     self.validator.run(model.cuda(), self.engine, curr_idx, self.tensorboard)
